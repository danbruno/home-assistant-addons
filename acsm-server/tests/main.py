...
sys.path.append("../calibre_plugin")
...
class TestAdobe(unittest.TestCase):
    ...
    def test_checkIfVersionListsAreValid(self):
        ...
        self.assertEqual(len(libadobe.VAR_VER_SUPP_CONFIG_NAMES), len(libadobe.VAR_VER_SUPP_VERSIONS),
                         "Version lists seem to be invalid")
        self.assertEqual(len(libadobe.VAR_VER_SUPP_CONFIG_NAMES), len(libadobe.VAR_VER_HOBBES_VERSIONS),
                         "Version lists seem to be invalid")
        self.assertEqual(len(libadobe.VAR_VER_SUPP_CONFIG_NAMES), len(libadobe.VAR_VER_OS_IDENTIFIERS),
                         "Version lists seem to be invalid")
        self.assertEqual(len(libadobe.VAR_VER_SUPP_CONFIG_NAMES), len(libadobe.VAR_VER_BUILD_IDS),
                         "Version lists seem to be invalid")
    ...
    def test_fingerprintGeneration(self):
        ...
        libadobe.devkey_bytes = bytearray(
            [0xf8, 0x7a, 0xfc, 0x8c, 0x75, 0x25, 0xdc, 0x4b, 0x83, 0xec, 0x0c, 0xe2, 0xab, 0x4b, 0xef, 0x51])

        self.assertEqual(libadobe.makeFingerprint("f0081bce3f771bdeeb26fcb4b2011fed77edff7b"),
                         b"FgLMNXxv1BZPqMOM6IUnfaG4Qj8=", "Wrong fingerprint")
        self.assertEqual(libadobe.makeFingerprint("HelloWorld123"), b"hpp223C1kfLDOoyxo8WR7KhcXB8=",
                         "Wrong fingerprint")
    ...
    def test_deviceKeyEncryption(self, random):
        ...
        random.get_random_bytes._mock_side_effect = lambda rndlen: bytearray(
            [0xc2, 0x3b, 0x0f, 0xde, 0xf2, 0x4a, 0xc3, 0x03,
    ...
    def test_hash_node_returnbugfix(self):
        ...
        self.assertEqual(sha_hash, "8b0a24ba37c4333d93650c6ce52f8ee779f21533",
                         "Invalid SHA hash for node signing (return)")
    ...
    def test_hash_node_ultralong(self):
        ...
        self.assertEqual(sha_hash, "7f62c1c1db2e1c965fd8403a4e768735a5848689",
                         "Invalid SHA hash for node signing (ultralong)")
    ...
    def test_sign_node_old(self):
        ...
        payload_bytes = bytearray(
            [0x34, 0x52, 0xe3, 0xd1, 0x1c, 0xdd, 0x70, 0xeb, 0x90, 0x32, 0x3f, 0x29, 0x1c, 0x06, 0xaf, 0xaf, 0xe1, 0x0e,
             0x09, 0x8a])
    ...
    def test_Account_loginCredentialEncryption(self):
        ...
        self.assertEqual(binascii.hexlify(msg), binascii.hexlify(expected_msg),
                         "devkey encryption returned invalid result")
...
class TestOther(unittest.TestCase):
    ...
    def test_pdf_cleanupEncrypt(self):
        ...
        self.assertEqual(cleanup_encrypt_element("ID[<1111><2222>]"), "ID[<1111> <2222>]",
                         "angle bracket spacing invalid")
        self.assertEqual(cleanup_encrypt_element("ID[  <1111> <2222>]  "), "ID[<1111> <2222>]",
                         "square bracket spacing invalid")
